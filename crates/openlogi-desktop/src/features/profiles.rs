...
pub(super) struct AddAppChoices {
    ...
    pub(super) applications: Vec<ProfileChoice>,
...
fn profile_scope_model(
    ...
    let available_catalog = catalog_state.available_profiles(&observed_ids, &unavailable);
    let choices = AddAppChoices {
        recent: available_recent,
        applications: available_catalog,
        loading: catalog_state.is_loading(),
        failed: catalog_state.is_failed(),
    };
    ProfileScopeModel {
        editing_app,
        profiles,
